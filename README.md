...
| `/rag/app` | SaaS App Studio Workspace — Chat Studio, Document Library, Search Inspector, Embed Configurator, Vector Sharding, ZKP Verifiable Grounding, and role-gated admin link |
...
The synchronizer entry point is `scripts/synchronizer.py`. Tab-specific views and helper functions reside under `scripts/sync_tabs/`. Shared safety helpers live in `scripts/sync_supabase.py`, `scripts/sync_json.py`, `scripts/sync_validation.py`, `scripts/sync_assets.py`, and `scripts/sync_git.py`. Additionally, `scripts/oracle_ampere_grabber.py` provides an automated Oracle Cloud Ampere host capacity grabber with multi-channel alerts (Telegram, Discord, Resend Email).
