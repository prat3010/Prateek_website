...
def scan_file(file_path: Path) -> list[tuple[int, str, str]]:
    findings = []
    # Skip test mocks and dummy keys
    ...
    for line_idx, line in enumerate(lines, start=1):
        ...
        # Skip comment documentation references
...
def main():
    ...
    print(f"🔒 Scanning {repo_root.name} for leaked secrets and high-risk credentials...")
    ...
    for path in repo_root.rglob("*"):
        ...
        if path.name.startswith(".env") and not path.name.endswith(".example"):
            # .env local files are gitignored, but don't fail scanning unless committed
            continue
