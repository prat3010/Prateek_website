...
- `src/components/` contains portfolio sections, shared UI (like the interactive diagnostics terminal console at `/info` which supports commands such as `git-info` and `qrcode`), visual effects, and the playground.
