...
- **Scoping as Authentic Retriever Tenant (Dogfooding Rule):** The Scoping Engine (`/scoping`) must be treated as a real, standard tenant of the Retriever platform. Prateek manually onboards the `prateeq_scoping` tenant in Retriever, uploads the scoping knowledge docs, and configures the LLM system prompt. Scoping connects to Retriever strictly via the API keys generated by Retriever (passed via environment variables like `RETRIEVER_SCOPING_TENANT_ID` and `RETRIEVER_SCOPING_API_KEY`, NEVER hardcoded). The Scoping chatbox uses the public embed widget script advertised on `/rag`.
- **Audit-First Retriever Integration Rule:** Before consuming or integrating ANY feature or artifact from Retriever into Scoping or the Website (such as the embed widget script, document upload API, streaming chat, or RLM math), the agent MUST first audit the implementation inside the `retriever` repository, verify that backend routes and tests pass, validate the payload contract, and only then integrate it into `Prateek_website`. Never write hacky workarounds on the website for backend flaws; fix them in `retriever` first.
- **Retriever Purity & Presentation Tier Decoupling Rule:**
  - `retriever` is a sovereign, standalone enterprise engine and must remain 100% pure and decoupled from `prateeq.in`.
  - **Never pollute `retriever` with website marketing logic:** Do NOT add website-specific marketing calculators, promotional competitor math, or `prateeq.in` domain references into `retriever`'s codebase.
  - **Frontend Ownership:** `Prateek_website` owns all custom presentation, sensory aesthetics (Design System 2.0), interactive widgets, and commercial marketing calculators on the frontend.
  - **Standard Tenant Consumption:** All RAG cognitive logic, vector searches, embeddings, and telemetry must be consumed strictly through `retriever`'s generic public REST/SSE endpoints (`/v1/tenants/{id}/search`, `/v1/tenants/{id}/chat`, `/v1/config/pricing`, `/widget.js`) using standard tenant credentials.
- **Automated Client Onboarding & 7-Day Trial (Locked Scope Contract):** When a user completes a project scope and authenticates to `/dashboard` (via Supabase Auth Google OAuth PKCE), the platform automatically provisions a dedicated tenant on Retriever (`tn_client_<uuid>`) with a 7-Day Trial plan and grants access to `/rag/app`. The details of the confirmed scope/SOW must be automatically compiled and indexed into the client's tenant as an immutable, permanent system document (`is_system: true`, `is_deletable: false`) visible in their Document Library, pre-grounding their workspace AI copilot while preventing deletion of the baseline contract.
- **Commercial vs Infrastructure Management Separation:** The local Streamlit Synchronizer (`scripts/synchronizer.py`, `sync_tabs/clients.py`) remains the single dedicated cockpit for viewing submitted client scopes, intake leads, invoices, deliverables, and linked Retriever tenant IDs. The Retriever Admin Dashboard is reserved exclusively for system tenant settings, vector parameters, and global token quotas.
- **Full Feature Flow Pre-Audit & Pre-Implementation Double-Audit:** Before writing or modifying code for any feature, state variable, or UI element, the agent MUST perform an exhaustive pre-implementation double-audit across target files and connected modules for ALL render/usage instances of the target state. The agent must verify transitive graph cascades, responsive viewports (<640px), containing block <Portal> safety, primary user interaction paths (sticky action bars, live input controls), and align third-party defaults with existing domain utility contracts (`pricing.ts`, `pdfTheme.ts`) before completing implementation.
...
- **Design System 2.0 & Sensory Aesthetics Invariant Rule:** Every future UI component, section, modal, drawer, or widget built or modified in this codebase MUST strictly preserve and match the established Design System 2.0 aesthetics:
  - **Dual-Theme Parity:** Full contrast and styling support across both **Azure** (graphic novel print, cold-press `#FAF9F6` paper, `#FFFFFF` cards, dark graphite `#2B2B36` text, terracotta `#E06D53`, slate blue `#3F6E91`) and **Noir** (cyber-monospace, `#08080a` obsidian glass, glowing cyan `#00f0ff`, neon green `#39ff14`).
  - **Zero Hardcoded Dark Palette Drift:** NEVER use hardcoded Tailwind dark hex colors (`#f8fafc`, `#94a3b8`, `#fff`, `#0f172a`, `rgba(255,255,255,...)`) in CSS modules or inline JSX styles. Always use semantic CSS tokens (`--color-text`, `--color-text-muted`, `--surface-card`, `--surface-elevated`, `--surface-glass-bg`, `--surface-glass-border`, `--badge-active-*`, `--app-btn-primary-*`).
  - **Sensory Kinetics Standards:** Primary CTAs must use `<MagneticButton strength={0.25}>`, feature showcase cards must use `<TiltCard maxAngle={2} glare={false}>`, dynamic pricing and financial numbers must use `@number-flow/react`, and tab indicators must use Framer Motion `m.span` spring layout pills (`layoutId`).
- **Production-First & Zero-Toy Utility Invariant Rule (No Mocks, No Gimmicks):** Every feature, API endpoint, UI component, and mathematical model MUST solve a real-world enterprise/commercial use case with 100% genuine end-to-end functionality.
  - **Zero Toyish Gimmicks:** Do NOT propose or implement superficial visual gimmicks (e.g. standalone spinning 3D particle screensavers or heavy Canvas animations) that add client-side GPU overhead without delivering tangible daily operational or analytical value to real users.
  - **Zero Mock Implementations & Hardcoded Placeholders:** NEVER write mock data branches, static `if/else` keyword checks simulating AI cognition, fake progress timers, or superficial UI facades in place of authentic backend engines. Everything must connect to real database tables, live Supabase/Retriever endpoints, authentic vector search, and verified business logic.
  - **Fail-Fast Invariant:** Never fake missing external services or unconfigured API credentials. If an external service or key is absent, throw `NotImplementedError` or return `HTTP 501 / 400` with an honest descriptive error. Never synthesize fake mock data to pretend an unconfigured integration is functioning.
  - **Automated Zero-Toy Gate (Gate 10):** Before finishing any task, `./scripts/verify.sh` must be executed and all 10 quality gates (including `python3 scripts/audit_zero_toy.py`) must report 100% green.
  - **High-Leverage Production Prioritization:** Always prioritize mission-critical production systems (zero-downtime deployment pipelines, automated load/latency benchmarks, empirical GraphRAG community clustering, and revenue-generating lead pipelines) over decorative flair.
  - **Fully Working & Verified:** Never declare a task complete unless every interaction path is fully wired, verified with real inputs, and backed by automated tests.
...
- **Web Application & Control Plane:** Fully deployed on Vercel, mapped to `prateeq.in`. Hosts the portfolio, `/scoping` engine, `/dashboard` client portal, and `/rag` product landing pages.
...
- **Retriever Admin Dashboard:** Deployed at **[`https://admin.rag.prateeq.in`](https://admin.rag.prateeq.in)** (`retriever/apps/web`). The dedicated control center for tenant onboarding (`/onboard`), API key issuance, document vector ingestion, and system prompt configuration.
...
- `src/app/api/` contains REST API routes for reading/writing portfolio & client data to Supabase: `skills`, `projects`, `certificates`, `profile`, `git-log`, `analytics-summary`, `contact`, `terminal/qrcode`, `terminal/query`, `terminal/snake-leaderboard`, `client/save-scope`, `client/get-scopes`, `client/delete-scope`, `client/copilot`, `client/change-orders`, `client/intake-draft`, `client/create-razorpay-order`, `client/verify-razorpay-payment`, `client/create-razorpay-subscription`, `client/create-razorpay-invoice`, `client/get-invoices`, `scoping/parse-intent`, `scoping/parse-rfp`, `scoping/validate-promo`, `rag/invite`, `rag/members`, `rag/telemetry`, `rag/tenant`, `outreach/dispatch`, `outreach/get-leads`, `outreach/prospect`, `webhooks/razorpay`, `blog/publish`, and `revalidate`. The client scope routes (`/api/client/*`) are session-gated: callers must pass `Authorization: Bearer <supabase access_token>`, and `get-scopes`/`save-scope` derive the client email from the verified session token (never from request parameters) via `src/lib/sessionVerify.ts`.
...
- `src/components/rag/` contains the interactive RAG Lab playground and SaaS App Studio panels: `ChatPanel.tsx`, `DocumentsPanel.tsx`, `SearchPanel.tsx`, `ConfigPanel.tsx`, `PricingSection.tsx`, `OverviewPanel.tsx`, `CachePanel.tsx`, `RlmStudioPanel.tsx`, `VectorVisualizerPanel.tsx`, `TeamPanel.tsx`, `ComparisonSection.tsx`, `DeveloperApiSection.tsx`, `InteractiveWidgetCustomizer.tsx`, and `ErrorBoundary.tsx` — all connected to the `retriever` backend via `src/lib/rag-client.ts`. Features include 👍/👎 message feedback (`submitFeedback`), clickable presigned citation downloads (`getDownloadUrl`), semantic cache badges (`⚡ Cached`), Geo-IP pricing (INR/USD), and Lenis smooth scroll isolation (`data-lenis-prevent`).
- `src/components/` contains portfolio sections, shared UI (like the interactive diagnostics terminal console at `/terminal` which supports commands such as `git-info` and `qrcode`, the `GestureScroll` floating gesture scroll controller, and the `IntakeForm` client discovery scoping questionnaire with instant PDF export), and visual effects. Components follow the pattern `src/components/{ComponentName}/{ComponentName}.tsx` with co-located CSS modules. The `src/components/Intake/` directory houses the decomposed scoping wizard sub-components (`AiScopingPromptBar.tsx`, `ArchitectureCartDrawer.tsx`, `ArchitectureTopologyMap.tsx`, `DependencyCascadeModal.tsx`, `QuickServiceFlow.tsx`, `RfpUploaderModal.tsx`, `ScopingChatWidgetDrawer.tsx`, `ServiceTypeGate.tsx`, `StepBrandKit.tsx`, `StepCommercials.tsx`, `StepGoalArchetype.tsx`, `StepTechnicalScope.tsx`, `StickyPriceBar.tsx`, and `useIntakeFormState.ts`).
...
- `src/lib/sessionVerify.ts` verifies Supabase session tokens for protected API routes.
- `src/lib/clientOrder.ts` handles client change order processing logic.
- `src/lib/emailNotification.ts` provides email notification templates for Resend.
- `src/lib/invoicing.ts` contains invoice generation and Razorpay payment utilities.
- `src/lib/onboardingChecklist.ts` manages the client onboarding workflow state machine.
- `src/lib/pathfindingAlgorithms.ts` implements pathfinding algorithms for the terminal pathfinder game.
- `src/lib/rag-types.ts` defines shared TypeScript interfaces for RAG components (`SearchResult`, `DocumentMeta`, etc.).
- `src/lib/security.ts` provides security primitives and validation utilities.
- `src/lib/terminalAudio.ts` manages terminal sound effects and audio playback.
- `src/lib/terminalPresentation.ts` implements the terminal presentation/slideshow engine.
- `src/lib/terminalScoping.ts` provides terminal-based scoping command handlers.
...
- `src/context/` contains global client providers: `ThemeContext.tsx` (theme and communication identity), `LenisProvider.tsx` (smooth scroll), `AuthContext.tsx` (Supabase Auth state), and `PerformanceGovernor.tsx` (device capability detection and animation budget).
- `src/hooks/` contains shared client hooks (e.g., typewriter effects, scrambled text). `src/hooks/dashboard/` houses dashboard-specific hooks: `useDashboardScopes.ts` and `useDashboardInvoices.ts`.
...
- `scripts/sync_tabs/` contains tab-specific view modules for the Streamlit dashboard (`analytics.py`, `resume.py`, `projects.py`, `certificates.py`, `skills.py`, `photos.py`, `blog.py`, `blog_service.py`, `clients.py`, `rag_pricing.py`, `outreach.py`, `questionnaire.py`, `retriever_query.py`, and `shared.py` common parsing/utilities).
...
- **Unit & Integration Tests:** Run `npm test` (Vitest) to execute all 355 tests across 46 test files. Uses `pool: 'forks'` for proper `process.env` isolation between test files.
...
- `src/lib/__tests__/` — Data layer, skill helpers, pricing, commission, security, PDF smoke tests, portal safety, terminal modules
- `src/app/api/__tests__/` — API route handlers (skills, projects, certificates, profile, contact, revalidate, git-log, analytics-summary, client scopes, copilot, change orders, razorpay, RAG invite/telemetry, scoping, snake-leaderboard, outreach, validate-promo)
- `src/hooks/__tests__/` — Client hooks (`useTypewriter`, `useScrambledText`)
- `src/app/analytics/__tests__/` — Analytics dashboard utilities
- `src/components/Intake/__tests__/` — Scoping wizard sub-components (AiScopingPromptBar, ArchitectureCartDrawer, ArchitectureTopologyMap, DependencyCascadeModal)
- `src/components/rag/__tests__/` — RAG panels (RlmStudioPanel, VectorVisualizerPanel)
- `src/components/effects/skyline/__tests__/` — Skyline interaction components (InteractiveGargoyle)
- `src/components/ui/__tests__/` — Shared UI components (Navbar)
- `src/context/__tests__/` — Context utilities (isJwtExpired)
