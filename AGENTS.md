...
- **Environment & DB Safety:** BEFORE adding, editing, or writing any new payload properties to Supabase tables, the agent MUST inspect live table schemas via `execute_sql` and execute any missing `ALTER TABLE ... ADD COLUMN IF NOT EXISTS ...` DDL migrations FIRST. Never write code that sends un-migrated columns to Supabase REST endpoints. Always verify database schema updates and ensure local JSON fallback synchronization before applying code changes.
