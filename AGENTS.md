...
- `src/data/` contains type definitions, Supabase client setup, taglines, and JSON fallback files (data values live in Supabase). `middlemanAgreementDefaults.json` holds the default prose for the Sales Partner Agreement (used by both PDF renderers and the synchronizer's reset button).
