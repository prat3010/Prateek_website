...
const mockChain = createResolvableChain({ data: [{ id: 1, name: 'Test Skill' }], error: null }) as Record<string, ReturnType<typeof vi.fn>>;
...
beforeEach(() => {
  ...
  mockChain.single.mockResolvedValue({ data: { id: 1, name: 'Test Skill' }, error: null });
});
...
describe('GET /api/skills', () => {
  it('returns skills data on success', async () => {
    const res = await GET();
...
describe('POST /api/skills', () => {
  ...
  it('returns 401 with wrong API key', async () => {
    const res = await POST(makePostRequest({ name: 'Test' }, 'wrong-key'));
    expect(res.status).toBe(401);
  });

  it('returns 201 on success with valid API key', async () => {
    const res = await POST(makePostRequest({ name: 'Test Skill' }, TEST_KEY));
