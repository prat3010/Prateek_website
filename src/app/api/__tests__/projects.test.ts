...
const mockChain = createResolvableChain({ data: [{ id: 1, title: 'Test Project' }], error: null }) as Record<string, ReturnType<typeof vi.fn>>;
...
beforeEach(() => {
  ...
  mockChain.single.mockResolvedValue({ data: { id: 1, title: 'Test Project' }, error: null });
});
...
describe('POST /api/projects', () => {
  ...
  it('returns 401 with wrong API key', async () => {
    const res = await POST(makePostRequest({ title: 'Test' }, 'wrong-key'));
    expect(res.status).toBe(401);
  });

  it('returns 201 on success', async () => {
    const res = await POST(makePostRequest({ title: 'Test Project' }, TEST_KEY));
