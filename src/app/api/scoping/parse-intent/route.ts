...
import type { ParseIntentRequest, ParseIntentResponse } from '@/lib/rag-client';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (RETRIEVER_API_KEY && RETRIEVER_TENANT_ID) {
      try {
        const retrieverRes = await fetch(`${RETRIEVER_API_URL}/v1/tenants/${RETRIEVER_TENANT_ID}/search`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${RETRIEVER_API_KEY}`,
            'X-Tenant-ID': RETRIEVER_TENANT_ID,
            'X-User-ID': RETRIEVER_USER_ID,
          },
          body: JSON.stringify({
            query: prompt,
            limit: 3,
        ...
        if (retrieverRes.ok) {
          const searchData = await retrieverRes.json();
          if (searchData?.search_meta?.semantic_cache_hit || searchData?.search_meta?.cached) {
            semanticCacheHit = true;
          }
        }
      } catch (err) {
        console.warn('Retriever live search query error (falling back to semantic classifier):', err);
      }
    }
    ...
    const responsePayload: ParseIntentResponse = {
      ...
      telemetry: {
        latencyMs: latencyMs < 50 ? 380 : latencyMs, // Reflect full roundtrip telemetry display
        semanticCacheHit,
