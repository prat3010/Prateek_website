...
import type { ResumeData, MiddlemanAgreementConfig, BaseEngineItem, FeatureItem, BrandAssetOption, MaintenancePlanOption } from '@/data/resume';
...
export interface UseTerminalCommandsOptions {
  projects: ProjectSummary[];
  profileData: ResumeData;
  isNoir: boolean;
  stats: SystemStats;
  setActiveGame: (game: 'none' | 'snake' | 'pathfinder') => void;
  isMatrixActive: boolean;
  setIsMatrixActive: React.Dispatch<React.SetStateAction<boolean>>;
}
...
export function useTerminalCommands({
  ...
  const [scopeSession, setScopeSession] = useState<TerminalScopeSession>(() =>
    createInitialTerminalScopeSession('INR')
  );
  ...
  const executeCommand = useCallback(
    (cmd: string) => {
      ...
      if (
        ...
        (interviewSession.step === 'role_select' &&
          (trimmedCmd === '1' || trimmedCmd === '2' || trimmedCmd.startsWith('1 ') || trimmedCmd.startsWith('2 ')))
      ) {
